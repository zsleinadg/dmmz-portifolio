...
function FloatingTag({ text, className, style }: { text: string; className?: string; style?: React.CSSProperties }) {
  return (
    <div
      className={`absolute font-mono text-xs lg:text-lg font-medium text-[rgba(167,139,250,0.7)] select-none animate-[floatTag_3s_ease-in-out_infinite] ${className ?? ""}`}
      style={{
        ...style,
        padding: "0.5rem 1rem",
        borderRadius: "1rem",
...
export function Hero() {
  return (
    <section
      id="about"
      className="min-h-screen bg-[#08090D] flex items-center relative overflow-hidden pt-17"
    >
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          backgroundImage:
            "linear-gradient(rgba(36,39,51,0.15) 1px, transparent 1px), linear-gradient(90deg, rgba(36,39,51,0.15) 1px, transparent 1px)",
        ...
        <div className="flex items-center justify-between gap-16 max-[900px]:flex-col max-[900px]:text-center">
          <div className="flex-1 min-w-0">
            ...
            <h1 className="font-bold leading-[1.1] mb-6 tracking-tight">
              <span
                className="block text-[clamp(48px,6vw,72px)] text-[#F5F5F7]"
              >
                Desenvolvedor
              </span>
              <span
            ...
            <p className="font-sans text-base lg:text-lg leading-relaxed text-[#A1A5B3] max-w-130 lg:max-w-150 mb-10 max-[900px]:mx-auto">
              Olá, me chamo <span className="text-white">Daniel Marques</span>. Sou um desenvolvedor Full-Stack focado em criar soluções completas, do design ao deploy, utilizando tecnologias atuais e boas práticas de desenvolvimento.
            </p>

            <div className="flex gap-3 mb-12 flex-wrap max-[900px]:justify-center">
              <button
                onClick={() => document.querySelector("#projects")?.scrollIntoView({ behavior: "smooth" })}
                className="inline-flex items-center gap-2 bg-[#7C3AED] text-[#F5F5F7] border-none rounded-lg px-6 py-3 text-sm font-semibold cursor-pointer transition-all duration-200 hover:bg-[#8B5CF6] hover:-translate-y-0.5"
              >
                Ver projetos
                <ArrowRight size={15} />
              </button>
              ...
              <a
                ...
                className="inline-flex items-center gap-2 bg-transparent text-[#F5F5F7] border border-[#242733] rounded-lg px-6 py-3 text-sm font-semibold cursor-pointer transition-all duration-200 hover:border-[#7C3AED] hover:-translate-y-0.5 no-underline"
              >
                Baixar CV
                <Download size={15} />
              </a>
            ...
            >
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-[#6F7482] no-underline font-medium transition-all hover:-translate-y-1 hover:text-red-700"
              >
                GitHub
                <GithubIcon />
              </a>
              <div className="w-px h-5 bg-[#242733]" />
              <a
                href="https://www.linkedin.com/in/danielmunizworks/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-[#6F7482] no-underline font-medium transition-all hover:-translate-y-1 hover:text-[#0077B5]"
              >
                LinkedIn
                <LinkedinIcon />
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-[#6F7482] no-underline font-medium transition-all hover:-translate-y-1 hover:text-indigo-600"
              >
                Currículo
                <FileText size={22} />
              </a>
